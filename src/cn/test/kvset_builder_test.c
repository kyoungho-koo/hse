...
MTF_DEFINE_UTEST_PREPOST(test, t_kvset_builder_add_entry1, pre, post)
{
    ...
    err = kvset_builder_add_val(bld, seq1, 0, 0, 0, NULL, 0);
    ASSERT_EQ(err, 0);
    err = kvset_builder_add_val(bld, seq1, vdata1, 0, 0, NULL, 0);
    ASSERT_EQ(err, 0);
    /* tombstone: vlen can be zero or non-zero */
    err = kvset_builder_add_val(bld, seq1, HSE_CORE_TOMB_REG, 0, 0, NULL, 0);
    ASSERT_EQ(err, 0);
    err = kvset_builder_add_val(bld, seq1, HSE_CORE_TOMB_REG, vlen1, 0, NULL, 0);
    ASSERT_EQ(err, 0);
    /* pfx tombstone: vlen can be zero or non-zero */
    err = kvset_builder_add_val(bld, seq1, HSE_CORE_TOMB_PFX, 0, 0, NULL, 0);
    ASSERT_EQ(err, 0);
    err = kvset_builder_add_val(bld, seq1, HSE_CORE_TOMB_PFX, vlen1, 0, NULL, 0);
    ASSERT_EQ(err, 0);
    /* real values */
    err = kvset_builder_add_val(bld, seq1, vdata1, vlen1, 0, NULL, 0);
    ASSERT_EQ(err, 0);
    err = kvset_builder_add_val(bld, seq2, vdata2, vlen2, 0, NULL, 0);
    ASSERT_EQ(err, 0);
    err = kvset_builder_add_val(bld, seq2, 0, 0, 0, NULL, 0);
    ASSERT_EQ(err, 0);
...
MTF_DEFINE_UTEST_PREPOST(test, t_kvset_builder_add_val_fail1, pre, post)
{
    ...
    err = kvset_builder_add_val(bld, seq, value, strlen(value), 0, NULL, 0);
    ASSERT_EQ(err, 1234);
...
MTF_DEFINE_UTEST_PREPOST(test, t_reserve_kmd2, pre, post)
{
    ...
    for (i = 0; i < 100; i++) {
        err = kvset_builder_add_val(bld, seq, "foobar", 6, 0, NULL, 0);
        if (err)
            break;
    }
    ASSERT_EQ(merr_errno(err), ENOMEM);
